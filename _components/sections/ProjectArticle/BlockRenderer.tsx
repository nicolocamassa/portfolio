...
const Heading = ({
  ...
  if (block.level === 1)
    return (
      <h2
        id={block.id}
        className={`${baseClass} mt-(--space-xs) text-2xl md:text-4xl!`}
      >
        {block.text}
      </h2>
    );
  if (block.level === 2)
    return (
      <h3
        id={block.id}
        className={`${baseClass} mt-(--space-md) text-xl md:text-2xl -mb-1`}
      >
        {block.text}
      </h3>
    );
...
const CalloutBlock = ({
  ...
  return (
    <Callout key={block.title} type={block.variant} title={block.title}>
      {block.text}
    </Callout>
  );
};
  ...
  return (
    <>
      {blocks.map((block, idx) => {
        switch (block.type) {
          case "heading":
            return <Heading key={idx} block={block}></Heading>;
          case "paragraph":
            return <Paragraph key={idx} block={block}></Paragraph>;
          case "image":
            return <Image key={idx} block={block}></Image>;
          case "list":
            return <List key={idx} block={block}></List>;
          case "code":
            return <Code key={idx} block={block}></Code>;
          case "callout":
            return <CalloutBlock key={idx} block={block}></CalloutBlock>;
          default:
            return null;
        }
      })}
